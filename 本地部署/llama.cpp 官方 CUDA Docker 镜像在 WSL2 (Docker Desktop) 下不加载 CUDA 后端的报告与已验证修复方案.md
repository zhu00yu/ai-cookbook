...
> 本报告基于 2026-09-22 ~ 2026-09-23 的完整实测排障，包含问题复现、证据链、根因分析和一套**已验证可用**的自建镜像方案。
...
| GPU | NVIDIA GeForce MX450（2GB 显存，Turing 架构 = **sm_75**） |
| NVIDIA 驱动 | 581.95（CUDA 13.0，容器 CUDA 12.8 向前兼容） |
| 容器架构 | x86_64（legacy CPU） |
...
| 模型 | MiniCPM5-1B-Q4_K_M.gguf（656MB） |
...
docker run --rm --gpus all -v /path/to/models:/models \
  ...
  -m /models/MiniCPM5-1B-Q4_K_M.gguf --host 0.0.0.0 --port 8080 -ngl 99
```
...
void ggml_backend_load_all() {
    ...
    ggml_backend_load_best("cuda", silent, dir_path);   // 尝试加载 libggml-cuda-*.so
    ...
    const char * backend_path = std::getenv("GGML_BACKEND_PATH");
    if (backend_path) {
        ggml_backend_load(backend_path);                // 手动指定插件
    }
}
```
...
# 关键修复1: GGML_BACKEND_DL=OFF -> CUDA 静态编进依赖链, 绕开运行时插件加载失效的坑
# 关键修复2: CMAKE_EXE_LINKER_FLAGS=-Wl,--allow-shlib-undefined -> 镜像无 libcuda.so 的链接问题
# 目标 GPU: NVIDIA MX450 (Turing, sm_75) -> CMAKE_CUDA_ARCHITECTURES=75 (按你的 GPU 架构调整)
...
FROM ${REGISTRY}/nvidia/cuda:${CUDA_VERSION}-devel-ubuntu${UBUNTU_VERSION} AS build
...
RUN cmake -B build -DGGML_NATIVE=OFF -DGGML_CUDA=ON -DGGML_BACKEND_DL=OFF \
        -DCMAKE_CUDA_ARCHITECTURES=75 -DGGML_CPU_ALL_VARIANTS=OFF \
...
FROM ${REGISTRY}/nvidia/cuda:${CUDA_VERSION}-runtime-ubuntu${UBUNTU_VERSION} AS runtime
...
> 说明：`GGML_BACKEND_DL=OFF` 后，llama-server 的 `ldd` 直接包含 `libggml-cuda.so`（静态依赖），不再依赖运行时插件加载。
...
### 7.3 验证结果（本机实测，MiniCPM5-1B-Q4_K_M）
...
- 本报告所有数字均为同一台机器、同一模型（MiniCPM5-1B-Q4_K_M.gguf）实测；速度取 `/completion` 返回的 `timings`。
- 环境差异注意：本机 GPU 为 Turing sm_75（MX450），`CMAKE_CUDA_ARCHITECTURES` 需按目标 GPU 调整（可用 `nvidia-smi --query-gpu=compute_cap --format=csv` 查询）。
